...
//             0    ,1312.2   , 477.4,
//             0     ,    0  ,  1);
//            Mat distCoeffL = (Mat_<double>(5, 1) <<-0.417649469041207,	0.421977434170464,	0.00485462022705676,	-0.00150186390817536,0);

//            Mat cameraMatrixR = (Mat_<double>(3, 3) <<
//                                 1318.0,    3.1  ,  714.8,
//             0  ,  1318.3    ,371.7,
//             0         ,0    ,1);
//            Mat distCoeffR = (Mat_<double>(5, 1) <<-0.408881080678867,	0.306614694242763,	0.00590016465505061	,0.00515564688906429,0);

//            Mat T = (Mat_<double>(3, 1) <<-194.168197967897	,-1.75959382624635,0.122314259231424);//T平移向量
//            Mat R = (Mat_<double>(3, 3) <<
//                     0.9999  , -0.0080  , -0.0140,
//        0.0082  ,  0.9998 ,   0.0158,
//        0.0139 ,  -0.0159 ,   0.9998);//R旋转向量
            /**************************************标定参数********************************************/
                Mat cameraMatrixL = (Mat_<double>(3, 3) <<
                                        1372.8 ,  -0.5    ,740.6,
                     0    ,1375.6    ,520.7,
                     0       ,  0    ,1 );
                    Mat distCoeffL = (Mat_<double>(5, 1) << -0.495889602777146,	0.477645986221552,-0.00585411774757405,	0.00303652771936167,0);

                    Mat cameraMatrixR = (Mat_<double>(3, 3) <<
                                         1378.5,   -4.6    ,724.9,
                     0    ,1383.3    ,416.9,
                     0     ,    0 ,   1);
                    Mat distCoeffR = (Mat_<double>(5, 1) <<-0.461471913080420,	0.190756195385854,-0.00502086207500832	,-0.00258682854378041,0);

                    Mat T = (Mat_<double>(3, 1) <<-206.8232 ,  -1.5990  ,  1.2477);//T平移向量
                    Mat R = (Mat_<double>(3, 3) <<
                            0.9997   ,-0.0099   ,-0.0229,
                0.0101  ,  0.9999  ,  0.0096,
                0.0228  , -0.0098   , 0.9997 );//R旋转向量
